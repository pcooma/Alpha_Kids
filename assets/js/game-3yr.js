...
const STRINGS = {
    en: {
        app_title: "Alpha Kids<br><span class=\"highlight\">Math Explorer (Age 3+)</span>",
        parents: "Parents",
        stickers: "🎒 Stickers",
        start: "START",
        win_sticker: "Win: ",
        ...
        guide_aim_text: "The aim of this app is to improve mathematical knowledge, logical thinking, and eye, hand, ear, and brain coordination for children in Age 3+.",
        ...
        guide_games_text: "<ul><li><b>Counting:</b> Numbers 1-5 with normal and mixed object variations.</li><li><b>Sorting:</b> 5 categories - Animals, Foods, Vehicles, Shapes, and Nature.</li><li><b>Find Odd:</b> Visual and category-based differences with shape variations.</li><li><b>Patterns:</b> ABAB, AABB, and ABC pattern sequences.</li><li><b>Compare:</b> Visual quantity comparison (more vs less).</li><li><b>Addition:</b> Visual sums up to 5 using familiar objects.</li></ul>"

    },
    si: {
        app_title: "Alpha Kids<br><span class=\"highlight\">ගණිත ගවේෂක (වයස 3+)</span>",
        parents: "දෙමාපියන්ට",
        stickers: "🎒 ස්ටිකර්",
        start: "පටන් ගමු",
        ...
        guide_aim_text: "මෙම යෙදුමේ අරමුණ වයස 3+ දරුවන්ගේ ගණිත දැනුම, තර්කන හැකියාව සහ අත, ඇස, කන සහ මොළය අතර සම්බන්ධීකරණය දියුණු කිරීමයි.",
        ...
        guide_games_text: "<ul><li><b>ගණන් කිරීම:</b> 1-5 දක්වා සාමාන්‍ය සහ මිශ්‍ර වෙනස්කම් සමඟ.</li><li><b>වෙන් කිරීම:</b> වර්ග 5ක් - සතුන්, කෑම, වාහන, හැඩතල සහ ස්වභාවික.</li><li><b>වෙනස් දේ සෙවීම:</b> පෙනුම සහ වර්ග පදනම් වන වෙනස්කම් සමඟ.</li><li><b>රටා:</b> ABAB, AABB සහ ABC රටා අනුක්‍රම.</li><li><b>සංසන්දනය:</b> පෙනුමෙන් ප්‍රමාණ සංසන්දනය (වැඩි/අඩු).</li><li><b>එකතු කිරීම:</b> හුරුපුරුදු වස්තු භාවිතයෙන් 5 දක්වා එකතු කිරීම.</li></ul>"
...
class EmojiBank {
    ...
    static NATURE = [
        ...
        { char: "🌈", en: "rainbow", si: "දේදුන්න" }, { char: "💧", en: "water drop", si: "ජල බිංදුව" }
    ];
...
class GameGenerator {
    static generateLevel(difficulty, forcedMode = null) {
        let modes = ['COUNT', 'FIND_ODD', 'SORT', 'PATTERN', 'COMPARE', 'ADD'];
        let mode = forcedMode;
        if (!mode || mode === 'RANDOM') {
            let available = ['COUNT', 'FIND_ODD', 'SORT'];
            if (difficulty > 3) available.push('PATTERN', 'COMPARE');
            if (difficulty > 5) available.push('ADD');
            mode = available[Math.floor(Math.random() * available.length)];
        }
        const maxNum = Math.min(2 + Math.floor(difficulty / 2), 5);
        const minNum = 1;

        // Variations logic
        let variation = 'normal';
        if (mode === 'COUNT') variation = Math.random() > 0.5 ? 'mixed' : 'normal';
        if (mode === 'PATTERN') {
            const r = Math.random();
            if (r < 0.33) variation = 'abab';
            else if (r < 0.66) variation = 'aabb';
            else variation = 'abc';
        }
        if (mode === 'FIND_ODD') {
            const r = Math.random();
            if (r < 0.4) variation = 'shape'; // New shape variation
            else variation = r > 0.7 ? 'category' : 'visual';
        }
        if (mode === 'SORT') {
            const r = Math.random();
            if (r < 0.3) variation = 'shape'; // New shape sort
            else variation = Math.random() > 0.5 ? 'living' : 'category';
        }
...
class GameScene extends Scene {
    ...
    setupCountGame() {
        const { target, variation } = this.levelData;
        const emoji = EmojiBank.getRandom();
        const itemName = EmojiBank.getName(emoji);
        ...
        // Initial instruction: "Count the X!" or "How many X?"
        const isLiving = EmojiBank.isLiving(emoji.char);
        let initText = TXT.instr_count(target, itemPlural, emoji.char, isLiving);
        ...
        let itemsList = [];
        // Add targets
        for (let i = 0; i < target; i++) itemsList.push({ ...emoji, isTarget: true });
        ...
        itemsList.forEach((itemData, idx) => {
            ...
            item.onclick = () => {
                ...
                this.app.audio.playSfx('pop');
                this.app.audio.speak(EmojiBank.getNumberWord ? EmojiBank.getNumberWord(counted + 1) : String(counted + 1));
                ...
                if (counted === target) {
                    ...
                    const isLiving = EmojiBank.isLiving(emoji.char);
                    const askText = TXT.instr_count_ask(itemPlural, emoji.char, isLiving);
                    instruction.innerText = askText;
                    this.setInstruction(askText);

                    const correctBtn = Array.from(optionsDiv.children).find(b => parseInt(b.innerText) === target);
        ...
        while (options.length < 3) {
            ...
            let minVal = Math.max(1, target - 2);
            let n = Math.floor(Math.random() * 5) + minVal;
        ...
        options.sort((a, b) => a - b).forEach(num => {
            ...
            btn.className = 'btn-tile option';
            btn.innerText = num;
            btn.onclick = () => this.checkAnswer(num === target, btn);
            optionsDiv.appendChild(btn);
        });
    ...
    setupCompareGame() {
        ...
        let rightCount;
        do { rightCount = Math.floor(Math.random() * 3) + 1; } while (rightCount === leftCount);
    ...
    setupFindOddGame() {
        ...
        if (variation === 'category') {
            ...
            const mainItems = EmojiBank.getRandomSet(mainCat, 3);
        ...
        } else if (variation === 'shape') {
            const main = EmojiBank.getRandom('SHAPES');
            let odd;
            do { odd = EmojiBank.getRandom('SHAPES'); } while (odd.char === main.char);
            const total = 4;
    ...
    setupSortGame() {
        ...
        if (variation === 'living') {
            ...
            cat1 = 'SHAPES';
            cat2 = 'ANIMALS';
            b1Label = TXT.cat_SHAPES;
            b2Label = TXT.cat_ANIMALS;
            icon1 = '🔴';
            icon2 = '🐶';
        } else {
            const cats = [];
        ...
        if (variation === 'living') {
            ...
            const shapes = EmojiBank.getRandomSet('SHAPES', 2);
            const animals = EmojiBank.getRandomSet('ANIMALS', 2);
            allItems = [...shapes, ...animals].sort(() => Math.random() - 0.5);
        } else {
        ...
        basketsRow.className = 'baskets-row';
        ...
        allItems.forEach((item, idx) => {
            ...
            let isTarget1 = false;
            if (variation === 'living') {
                isTarget1 = EmojiBank.isLiving(item.char);
            } else if (variation === 'shape') {
                isTarget1 = EmojiBank.SHAPES.some(e => e.char === item.char);
            } else {
                isTarget1 = EmojiBank[cat1].some(e => e.char === item.char);
            }

            el.dataset.type = isTarget1 ? '1' : '2';
            ...
            el.onclick = () => {
                ...
                const correctBasket = type === '1' ? b1 : b2;
